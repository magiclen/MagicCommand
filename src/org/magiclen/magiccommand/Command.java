...
public class Command {
    ...
    public void runAsync(final String id, final File directory) {
        new Thread() {
            @Override
            public void run() {
                Command.this.run(id, directory, true);
            }
        }.start();
    }
     ...
     * @param id 傳入命令的ID字串
     * @param directory 傳入工作目錄物件
     * @param async 傳入是否以非同步模式來呼叫
     */
    private void run(String id, final File directory, final boolean async) throws RuntimeException {
        ...
        synchronized (this) {
            ...
            if (id == null) {
                ...
                final RuntimeException ex = new RuntimeException(String.format("ID \"%s\" is using.", id));
            ...
            try {
                ...
                final RuntimeException ex2 = new RuntimeException(ex.getMessage());

                if (async && listener != null) {
                    listener.commandException(id, ex2);
